...
class Editorial extends Module
{
	...
	function getContent()
	{
		...
		if (isset($_POST['submitUpdate']))
		{
			...
			/* upload the image */
			if (isset($_FILES['body_homepage']) AND isset($_FILES['body_homepage']['tmp_name']) AND !empty($_FILES['body_homepage']['tmp_name']))
			{
				Configuration::set('PS_IMAGE_GENERATION_METHOD', 1);
				if ($error = checkImage($_FILES['body_homepage'], $this->maxImageSize))
					$errors .= $error;
				elseif (!$tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS') OR !move_uploaded_file($_FILES['body_homepage']['tmp_name'], $tmpName))
					return false;
				elseif (!imageResize($tmpName, dirname(__FILE__).'/homepage.jpg'))
